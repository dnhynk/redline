...
# 주입받으므로 숫자를 두 곳에 적지 않는다. 진입점도 이 값을 가져다 써야 한다 —
# 기본값과 진입점 상수가 갈라져 있으면 "숫자를 두 곳에 적지 않는다"가 거짓말이 된다.
...
_TERMINAL_PUNCT = ".!?。！？…"
# 종결부호 뒤에 따라붙을 수 있는 닫는 부호 — 문장 경계는 이것들 **뒤**다.
_CLOSE = r"[\"'”’〞」』】〉》）)\]]"
# 네 갈래인 이유는 둘이다.
#
# 1. **공백을 요구할 수 있는 종결부호와 없는 종결부호가 다르다.**
#    반각 `.!?…`는 뒤따르는 공백을 요구한다 — 그 공백이 소수점("7.2년")과 약어를 문장
#    경계에서 지켜준다. 전각 `。！？`는 중국어·일본어에서 뒤에 공백을 두지 않으므로
#    공백을 요구하면 한 단락이 통째로 문장 하나가 되고, 그 위에 선 좌표계·커버리지·
#    하이라이트가 전부 무너진다. 전각 부호는 소수점으로 쓰이지 않아 공백 없이 잘라도
#    오분할하지 않는다.
# 2. **닫는 부호는 앞 문장의 것이다.** 구분자로 삼켜 버리면 「…です。」의 `」`가 좌표계에서
#    사라진다 — 화면은 이 배열을 원문으로 그리므로 글자가 없어지면 안 된다. 그래서
#    닫는 부호가 뒤따르는 자리에서는 자르지 않고(`(?!닫는부호)`), 그 부호 **뒤**에서 자른다.
